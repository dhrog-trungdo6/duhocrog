...
| 13 | `20260713000013` | Majors N-N: bảng `majors` (slug unique, name_vi/name_en, category, is_active) + junction `school_majors` (PK kép, FK cascade); RLS public SELECT active-only (link check EXISTS 2 chiều), ghi = service role; index `(major_id, school_id)` + partial category. GIỮ cột `schools.programs` JSONB — migrate data sau | ✅ Applied (2026-07-12) |
| 14 | `20260713000014` | Seed 39 ngành chuẩn (bản địa hóa Capstone/ApplyBoard) gom 9 nhóm category — on conflict slug do nothing | ✅ Applied (2026-07-12, verify /api/majors 39 ngành). ⚠️ school_majors vẫn 0 link — filter theo ngành trả rỗng đến khi migrate programs→links |
